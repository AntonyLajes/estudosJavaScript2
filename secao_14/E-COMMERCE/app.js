...
function validaSenha(){
    const regex = /^(?=.*[A-Z])(?=.*[a-z])(?=.*[0-9])(?=.*[!#@%&]).{6,15}$/; //{6,15} define o maximo e o minimo de caractere em um regex - ?=.* define condições para o busca(pesquisar mais sobre)
...
function validaData(){
    const regex = /^(?<ano>\d{4})[/](?<mes>(0[1-9]|1[0-2]))[/](?<dia>(0[1-9]|12[0-9]|3[01]))$/;
